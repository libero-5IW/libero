...
import { ContractTemplateVariable } from '@prisma/client';

@Injectable()
export class ValidateContractOnCreatePipe<
  T extends {
    templateId?: string;
...
> implements PipeTransform<T>
{
  ...
  private async getTemplateVariables(
    templateId: string,
  ): Promise<ContractTemplateVariable[]> {
    ...
    if (!template) {
      throw new BadRequestException(
        'Le modèle de contrat spécifié est introuvable.',
      );
    }
  ...
  private validateVariables(
    templateVariables: ContractTemplateVariable[],
    submittedVariables: CreateContractVariableValueDto[],
  ) {
